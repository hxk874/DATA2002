...
4. For each of the re-sampled (permuted) data, perform a two-sample t-test to extract the test statistic. Although we perform the t-test, we’re not really using the t-distribution. We’re just borrowing the test statistic not caring about its distribution $\to$ Non-parametric  

5. Compute the proportion of test statistics, as or more extreme than the original t-test statistic $\to$ a permuted p-value
...
Visualize the result. Hint: Write a for loop (see example in lecture slides).
...
**Perform a permutation test by generating 10,000 resamples. What conclusion do you reach based on the permutation test p-value?**
...
e. **Which of the procedures above provides the more appropriate inference of the population mean. Why?**
...
f. **Generate 95% bootstrap confidence intervals for the standard deviation and the median absolute deviation from the median (MAD). Plot histograms of both bootstrap distributions. Which of these two estimators of scale is more reliable in this setting.**
...
g. **Say we were interested in the coefficient of variation, $CV = s / \bar{x}$, for this data set. Generate a 90% bootstrap confidence interval for the coefficient of variation.**
...
a. **Produce some basic summary statistics the nutrients “sugar” and “sodium”**
```{r}
# summary(cereal)
```
