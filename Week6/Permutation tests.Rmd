...
For paired tests: think difference $d_i=x_i - y_i$
and then the test is the $sign(d_i)$
...
```{r}
# Different permutations of sign of 11 d_i.
...
n = length(d)
B = nrow(sign_permute) # all different sign
t_null = vector("numeric", B)

for(i in 1:nrow(sign_permute)){
  d_permute = d*sign_permute[i,] # permuted difference vector: org diff * permuted signs
  t_null[i] = mean(d_permute)/sd(d_permute)*sqrt(n) # test static under this null dist
}

# two taail type setting
